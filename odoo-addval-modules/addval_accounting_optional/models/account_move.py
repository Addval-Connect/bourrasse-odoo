...
class AccountMove(models.Model):
    ...
    def _l10n_cl_edi_post_validation(self):
        if not self._l10n_cl_edi_currency_validation():
            raise UserError(
                _(
                    "It is not possible to validate invoices in %s for %s, please convert it to CLP"
                )
                % (self.currency_id.name, self.l10n_latam_document_type_id.name)
            )
        if (
            self.l10n_cl_journal_point_of_sale_type == "online"
            and not (
                (
                    self.partner_id.l10n_cl_dte_email
                    or self.commercial_partner_id.l10n_cl_dte_email
                )
                and self.company_id.l10n_cl_dte_email
            )
            and not self.l10n_latam_document_type_id._is_doc_type_export()
            and not self.l10n_latam_document_type_id._is_doc_type_ticket()
        ):
            raise UserError(
                _(
                    "The %s %s has not a DTE email defined. This is mandatory for electronic invoicing."
                )
                % (
                    _("partner")
                    if not (
        ...
        # Comented out because user decided to not invalidate future dates on invoices
        # if (
        #     datetime.strptime(
        #         self._get_cl_current_strftime(), "%Y-%m-%dT%H:%M:%S"
        #     ).date()
        #     < self.invoice_date
        # ):
        #     raise UserError(
        #         _(
        #             "The stamp date and time cannot be prior to the invoice issue date and time. TIP: check "
        ...
        if self.l10n_latam_document_type_id.code not in [
            "39",
            "41",
            "110",
            "111",
            ...
            self.partner_id.l10n_cl_activity_description
        ...
        if (
            not self.l10n_latam_document_type_id._is_doc_type_electronic_ticket()
            and not self.partner_id.street
        ):
            raise UserError(
                _(
        ...
        if (
            self.l10n_latam_document_type_id.code in ["34", "41", "110", "111", "112"]
            and self.amount_untaxed != self.amount_total
        ):
            raise UserError(
                _(
                    "It seems that you are using items with taxes in exempt documents in invoice %s - %s."
                    " You must either:\n"
        ...
        if (
            self.l10n_latam_document_type_id.code == "33"
            and self.amount_untaxed == self.amount_total
        ):
            raise UserError(
                _(
                    ...
                    " or at least one of the items should have vat tax."
                )
                % (self.id, self.name)
            )
